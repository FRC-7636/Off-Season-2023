...
public class DriveSubsystem extends SubsystemBase {
   ...
   */
  public void drive(double xSpeed, double ySpeed, double rot, boolean fieldRelative,boolean isfast) {
    // deadzone
    if(Math.abs(xSpeed) < 0.45 && Math.abs(ySpeed) < 0.45 && Math.abs(rot) < 0.45) {
      xSpeed = 0;
      ySpeed = 0;
      rot = 0;
    }
    ...
    xSpeed= xSpeed*rate*  (isfast?n:1);
    ySpeed= ySpeed*rate*  (isfast?n:1);
    rot = rot *rate *  (isfast?n:1);
...
double x, y;
  public void level(){
    m_Timer.start();
    double init_timer = m_Timer.get();
    
while (Math.abs(m_gyro.getRoll())>3){   
  
   if (m_gyro.getRoll() > 7){
      y=-0.5;
    } 
    else if (m_gyro.getRoll() <-7){
      y=0.5;
    }
    else y=0;
    ...
    else if (m_gyro.getYaw() <-1){
      x=0.05;
    }
    else x=0;
  
if(m_Timer.get()-init_timer>15){
  break;
}
  drive(y, 0, x, true,false);
  }
